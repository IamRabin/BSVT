...
class CCT(pl.LightningModule):
    ...
    def __init__(
        ...
        heads: int = 4, emb_dim: int = 64, feat_dim: int = 2*64,temporal:int=118, 
        dropout: float = 0.1, attention_dropout: float = 0.1, layers: int = 4, 
        channels: int = 1, image_size: int = (19,500), num_class: int = 2
        ):
        super().__init__()
        ...
        self.tokenizer = ConvTokenizer(channels=1)

        with torch.no_grad():
            x = torch.randn([1,channels,temporal, image_size[0], image_size[1]])
    ...
    def forward(self, x: torch.Tensor):     
        ...
        x = rearrange(
            ...
            x, 'bs e_d t ph_h ph_w -> bs ( t ph_h ph_w) e_d ', 
...
if __name__=="__main__":
   ...
   with torch.cuda.device(0):
            macs, params = get_model_complexity_info(t, (1,118,19,500), as_strings=True, print_per_layer_stat=False)
